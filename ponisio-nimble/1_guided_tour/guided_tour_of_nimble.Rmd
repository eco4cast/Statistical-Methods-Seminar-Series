...
dOcc_code <- nimbleCode({
  ...
  for (site in 1:nsite){
    z[site,1] ~ dbern(psi1)
    for (year in 2:nyear){
      logit(muZ[site, year]) <- z[site,year-1]*phi[site, year-1] +
  ...
  for (site in 1:nsite){
    for (survey in 1:nsurvey){
      for (year in 1:nyear){
        muy[site,survey,year] <- z[site,year]*ilogit(p[site, survey, year])
        y[site,survey,year] ~ dbern(muy[site,survey,year])
      }
    }
  }
