...
To compare the relative sensitivity of eDNA and traditional sampling, we can use `detection_plot()` to find the units of survey effort necessary to detect presence of the species with $X$ probability. 
...
The below function is calculating the number of water samples and seine net samples needed to detect goby presence (i.e., at least one true positive eDNA detection or at least one individual in a seine sample) with 0.9 probability, given $\mu$.
